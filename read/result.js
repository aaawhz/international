    ...
    var s = setInterval(function(){
        if (parent.LMD.loadStatus == 1) {
            ...
            var deptLi = "<li id='li_{3}_{0}'><span class='fw_b pl_27 departList_span'   onclick=\"" + p.contact + ".{1}('{0}',{3},1)\">"
                ...
                +"{4}</div>"
                +"</span></li>";
            var addAllContactHtml_temp ="<i  class='addgroup' title='"+top.Lang.Mail.Write.tianjiaquanbu+"' deptId='{0}' type='{1}'     style='margin-right: 5px;float: right;margin-top: -18px;display:none'></i>";//<i  class='addgroup' title='添加全部' deptId='{0}' type='{1}'     style='margin-right: 5px;float: right;margin-top: -18px;display:none'></i>
...
Contact.prototype.createBody = function( showType , callback , action)
{
    ...
    {
        CC.alert(top.Lang.Mail.Write.ninmeiyouquanxianshiyonggaigongnen);//您没有权限使用该功能
        return;
    }
...
Contact.prototype.getHTML = function(flag){
    ...
    if(this.config&& this.config.maxCount){
        countLimitTips='<div class="tipsTxt" style="color: #fc0000;display:none;"  id="maxLimit">'+top.Lang.Mail.Write.zhixianyuxuanze+ this.config.maxCount+''+top.Lang.Mail.Write.geyonghu+'</div>'
    }//<div class="tipsTxt" style="color: #fc0000;display:none;"  id="maxLimit">只限于选择  ||  个用户</div>
    ...
    this.clearonclick = setTimeout(function(){
        ...
        if (groupMap[type+"_"+id] && type == "0") {//得到该节点下的子部门
            for (var i = 0; i < groupMap[type+"_"+id].length; i++) {
                ...
                {
                    if(p.addAsGroup){
                        addAllContactHtml ="<i  class='addgroup' title='"+top.Lang.Mail.Write.tianjiaquanbu+"' deptId='"+cId+"' type='"+type+"'     style='margin-right: 5px;float: right;margin-top: -18px;display:none'></i>";//<i  class='addgroup' title='添加全部' deptId='
                    }
                    else{
                        addAllContactHtml = "";
                    }
...
Contact.prototype.showChildLinkMan_Asyn=function(id, type, index,isFirstLoad)
{
    ...
    this.clearonclick = setTimeout(function(){
        ...
        for (var i = 0; i < orgList.length; i++) {
            var cId = orgList[i][1];
            var addAllContactHtml ="<i  class='addgroup' title='"+top.Lang.Mail.Write.tianjiazhengzu+"' deptId='"+cId+"' type='0'  style='margin-right: 5px;float: right;margin-top: -18px;display:none'></i>";//<i  class='addgroup' title='添加整组' deptId='
            if(!p.addAsGroup){
                addAllContactHtml = "";
            }
            html.push(p.deptList_temp.format(
...
Contact.prototype.go = function(obj,id,index,a_style,conObj)
{
    ...
    function callBack1(data){
        ...
        if (data.code == "S_OK") {
            ...
            var containerId = "#ul_0_" + id
            var nextHtml ="<li class='ta_c' style='overflow:hidden; padding-bottom:5px;'><a href='javascript:;' style='padding-right:0; color:#003be2;' id='"+id+"' index='"+(index+1)+"' a_style='"+a_style+"' class='nextPage' >"+top.Lang.Mail.Write.dianjijiazaigengduo+"</a></li>";//' class='nextPage' >点击加载更多>>  </a></li>
            if(jQuery(containerId).length>0){
