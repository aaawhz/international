...
var contact=
{
    ...
    showLinkMan:function(showType,divContentId,lang)
    {
        ...
        if(parent.LMD.loadStatus==1)
        {
            ...
            var deptLi = "<li id='li_{3}_{0}'><span class='fw_b pl_27 departList_span'   onclick=\"contact.{1}('{0}',{3},1)\">"
                        ...
                        +"{4}</div>"
                        +"</span></li>";
            var addAllContactHtml_temp ="<i  class='addgroup' title='"+top.Lang.Mail.Write.tianjiaquanbu+"' deptId='{0}' type='{1}'     style='position: absolute; right: 2px; top: 1px; display: none;'></i>";//<i  class='addgroup' title='添加全部' deptId='{0}' type='{1}'     style='position: absolute; right: 2px; top: 1px; display: none;'></i>
    ...
    showChildLinkMan:function(id,type,index)
    {
        ...
        if (groupMap[type+"_"+id] && type=="0") {//得到该节点下的子部门
            ...
            for (var i = 0; i < groupMap[type+"_"+id].length; i++) {
                ...
                {
                    addAllContactHtml ="<i  class='addgroup' title='"+top.Lang.Mail.Write.tianjiaquanbu+"' deptId='"+cId+"' type='"+type+"'     style='position:absolute;right:2px;top:1px;display:none'></i>";//<i  class='addgroup' title='添加全部' deptId='
                }
    ...
    showChildLinkMan_Asyn: function(id, type, index,isFirstLoad){
        ...
        this.clearonclick = setTimeout(function(){  
            ...
            var groupMap = parent.LMD.groupMap;;
            var a_style ="padding-left:"+(27+(index*16))+"px";
            var addAllContactHtml ="<i  class='addgroup' title='"+top.Lang.Mail.Write.tianjiazhengzu+"' deptId='{0}' type='0'  style='position: absolute; right: 2px; top: 1px; display:none'></i>";//<i  class='addgroup' title='添加整组' deptId='{0}' type='0'  style='position: absolute; right: 2px; top: 1px; display:none'></i>
