...
(function () {
    ...
    RemoteImage.prototype = {
        ...
        setPreview: function(){
            var url = encodeHtml($G('url').value),
                ow = encodeHtml($G('width').value),
                oh = encodeHtml($G('height').value),
                border = encodeHtml($G('border').value),
                title = encodeHtml($G('title').value),
                preview = $G('preview'),
                width,
                height;
    ...
    UploadImage.prototype = {
        ...
        initUploader: function () {
            ...
            // WebUploader实例
                uploader,
                actionUrl = "/RmWeb/mail?func=attach:upload&sid="+top.gMain.sid + "&type=internal";//editor.getOpt('imageUrl'),
            ...
            uploader.on('all', function (type, files) {
                switch (type) {
                    ...
                    case 'startUpload':
                        /* 添加额外的GET参数 */
                         var url = actionUrl+"&composeId="+(parent.oWrite?parent.oWrite.id :parent.Meeting.id);
            ...
            uploader.on('uploadSuccess', function (file, ret) {
                var $file = $('#' + file.id);
                try {
                    var responseText = (ret._raw.match(/\{.*}/g)[0] || ret),
                        json = utils.str2json(responseText.replace(/'/g,'"'));
                    if (json.code == 'S_OK') {
        ...
        getInsertList: function () {
            ...
            for (i = 0; i < this.imageList.length; i++) {
                ...
                list.push({
                    src: "/RmWeb/view.do?func=attach:getAttach&sid="+top.gMain.sid+"&fileId="+data["var"].fileId+"&fileName="+data["var"].fileName,
                    _src: "/RmWeb/view.do?func=attach:getAttach&sid="+top.gMain.sid+"&fileId="+data["var"].fileId+"&fileName="+data["var"].fileName,
                    title: data.title,
                    alt: data.original,
                    floatStyle: align
                });
