...
DiskDialog.prototype ={
    ...
    create:function(callback,showType){
        ...
        }, top.Lang.Mail.Write.xuanzewenjian,null,"diskListContrOl","565");    //,"350"//选择文件
        p.bindEvent();
    },
    _getheadHtml:function(){
        var temp = '<div class="img_tabTop"><ul style="height:28px;" class="ml_10 fl"><li id="tempDisk_li_up" type="disk" class="diskControlMenu r '+(this.showType==0?"current":"")+'">'+top.Lang.Mail.Write.wangpanwenjian+'</li>';//">网盘文件</li>
            temp +='<li id="tempDisk_li_list" type="tempDisk" class="diskControlMenu '+(this.showType==1?"current":"")+'">'+top.Lang.Mail.Write.wenjianzhongzhuanzhan+'</li></ul></div>';//">文件中转站</li></ul></div>
        return temp;        
    },
    /***
     * 没有文件HTML
    ...
    _getNoFileHtml:function(){
        var html =[];
          ...
          html.push('<div style="" id="temp_notData" class="rebackwrap"><p>'+top.Lang.Mail.Write.zwwjnkycdnxzwjtjscAbpDOkXi+'</p><p class="pt_5"></p></div>');//<div style="" id="temp_notData" class="rebackwrap"><p>暂无文件，你可以从电脑选择文件添加上传</p><p class="pt_5"></p></div>
          html.push('</div></div>')
          return html.join("");
    },
    /***
    ...
    getTempDiskHtml:function(data){
        ...
        this.queryTempDiskList(function(data){
            ...
            if(dataList.length==0){
                ...
                html.push('<a><em style="display: inline-block; width: 14px;"></em>'+top.Lang.Mail.Write.wenjianming+'</a></th>');//<a><em style="display: inline-block; width: 14px;"></em>文件名</a></th>
                html.push('<th style="width:70px" class="size"><a>'+top.Lang.Mail.Write.daxiao+'</a></th>');//<th style="width:70px" class="size"><a>大小</a></th>
                html.push('<th style="width:136px;border-right:medium none;" class="time"><a>'+top.Lang.Mail.Write.shengyushijian+'</a>');//<th style="width:136px;border-right:medium none;" class="time"><a>剩余时间</a>
    ...
    _getSurplusTime:function(shareTime){
        ...
        if (d > 0) {
            ...
            timeStr = d + top.Lang.Mail.Write.tian + h + top.Lang.Mail.Write.xiaoshi;//天  ||  小时
        }
        else {
            h = parent.Util.DateDiffMore(curtime,shareTime, "h");
            if (h > 0) 
                timeStr = h + top.Lang.Mail.Write.xiaoshi;//小时
            else {
                var n = parent.Util.DateDiffMore(curtime, shareTime, "n");
                if (n > 0) 
                    timeStr = n + top.Lang.Mail.Write.fenzhong;//分钟
                else 
                    timeStr = top.Lang.Mail.Write.buzufenzhong
            }//不足1分钟
        }
        return timeStr;
    },
