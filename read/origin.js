...
var task = {
    ...
    initListHome:function(){
        ...
        //if(pageVm.roles[0]){
            var sendVm = avalon.define({
                ...
                $aSearchStatus:[{text:'全部事项',value:0},{text:'未审批',value:1},{text:'完成审批',value:3},{text:'审批中',value:2},{text:'被驳回',value:4}],
                search:function(){
                    sendVm.$renderList()
                },
                ...
                createTask:function(){
                    top.CC.goOutLink( '../../workflow/createhome.do?sid='+ top.gMain.sid, 'createhome', '新建审批');
                },
        ...
        //if(pageVm.roles[1]){
            var handleVm = avalon.define({
                ...
                id: "myHandle",
                search_status:0,
                search_placeholder:'请输入事项名称或发起人搜索',
                $firstLoad:true,
                searchKey:'',
                total: 0,
                processing: 0,
                ...
                //search_status:0,
                $aSearchStatus:[{text:'全部事项',value:0},{text:'未审批',value:1},{text:'完成审批',value:3},{text:'驳回',value:4},{text:'审批中',value:2}],
                search:function(){
                    handleVm.$renderList()
                },
                preSearch:function(event){
        ...
        //if(pageVm.roles[2]){
            var checkVm = avalon.define({
                ...
                readed:0,
                listItems: [{}],
                $aSearchStatus:[{text:'全部事项',value:0},{text:'未查阅',value:1},{text:'已查阅',value:2}],
                search:function(){
                    checkVm.$renderList()
                },
                preSearch:function(event){
    ...
    initCreateGeneral:function(){
        ...
        function validateForm(){
            ...
            if(!vm.subject){
                task.validateAlert('请输入审批事项类型或主题')
                return false
            }
            if(!vm.title){
                task.validateAlert('请输入审批标题')
                return false
            }
            if(!vm.content){
                task.validateAlert('请输入审批事项说明')
                return false
            }
            if (task.uploader.getStats().progressNum != 0) {
                this.canSubmit = false
                parent.CC.showMsg('还有文件正在上传', true, false, 'error')
                return false
            }
            return true
        }
    ...
    initCreateAskForLeave:function(){
        ...
        function validateForm(){
            ...
            if(!vm.content){
                task.validateAlert('请输入审批内容说明')
                return false
            }
            if(!vm.holidayType){
                task.validateAlert('请选择请假类型')
                return false
            }
            if(!/^\d+(\.\d+)?$/g.test(vm.dayNum)){
                task.validateAlert('请输入正确的请假天数')
                return false
            }            
            if(!(vm.date_start&&vm.date_end&&vm.hour_start&&vm.hour_end)){
                task.validateAlert('请选择请假时间')
                return false
            }
            if(vm.date_start+vm.hour_start>vm.date_end+vm.hour_end){
                task.validateAlert('开始时间不能大于结束时间')
                return false
            }
            if (task.uploader.getStats().progressNum != 0) {
                this.canSubmit = false
        ...
        var vm=avalon.define({
            ...
            submit:function(){
                ...
                data = {
                    ...
                    "templateId":tplId,
                    "bizSubject": vm.subject,
                    "attrs": vm.dayNum + "天(" + data_start_temp + "至" + data_end_temp + ")",
                    "bizDesc": vm.bizDesc,
                    "attachmentList": task.attachmentList,
                    "contentJson":{
                        "content": vm.content,
    ...
    initCreateWorkOverTime:function(){
        ...
        function validateForm(){
            ...
            if(!/^\d+(\.\d+)?$/g.test(vm.dayNum)){
                task.validateAlert('请输入正确的加班天数')
                return false
            }
            if(!/^\d+(\.\d+)?$/g.test(vm.hourNum)){
                task.validateAlert('请输入正确的加班小时数')
                return false
            }
            // if(!vm.examineUserId){
 ...
 */
    ...
    initCreateWorkflow:function(){
        ...
        var vm=avalon.define({
            ...
            smsNotify:false,
            cancel:function(){
                if (confirm("关闭工单编辑页，内容将丢失，是否要关闭？")) {
                    parent.CC.exit();
                }
            },
            submit:function(){
...
//{"taskId":3,"businessId":17,"examineUserId":"admin@139.com","examineUserName":"admin","nodeId":1,"pass":true,"suggestion":"符合要求"}
    ...
    deleteTask:function(vm){
        if(confirm('确认删除审批？')){
            ...
            var config = {
                ...
                call: function (resp) {
                    var p = parent;
                    parent.CC.showMsg('删除成功', true, false, 'option')
                    // parent.GE.tab.close("outLink_oatask")
                    // var tabId=parent.CC.getCurLabId()
                    // parent.GE.tab.close(tabId)
                    parent.CC.exit()
                    p.CC.goOutLink( '../../workflow/listcheckworkflow.do?sid='+ p.gMain.sid, 'oatask', 'OA审批')
                },
                failCall: task.failCallback,
                params: {
                },
    ...
    //查看我发起的
    readMySend:function(){
        top.CC.goOutLink( '../../workflow/listcheckworkflow.do?sid='+ top.gMain.sid, 'oatask', 'OA审批')
    },
    ...
    getPreviewLink: function (file) {
        ...
        if (imgextensions.indexOf(file.extension) >= 0) {
            var str = '<a previewType="image" fileName="'+file.name.encodeHTML()+'" imageIndex="'+task.imageIndex+'" href="javascript:void(0);" onclick="task.previewImg(this);" href="' + task.getPreviewLinkHref(file) + '"  class="ml_5" ">在线预览</a> '
            task.imageIndex++;
            return str;
        }
        if (extensions.indexOf(file.extension) >= 0) {
            var str = '<a target="_blank" href="' + task.getPreviewLinkHref(file) + '" class="ml_5" >在线预览</a> '
            return str
        } else {
            return ''
        }
        ...
        jQuery(function () {
            ...
            task.uploader.on('beforeFileQueued', function (file) {
                ...
                if (file.size > 50 * 1024 * 1024) {
                    var tooBig = '<span class="tooBig" style="">文件超过50M，不能上传</span>'
                    $('#fileList').append('<p id="fileId_' + file.id + '"><span class="fileName">' + file.name + '</span><span class="process">' + tooBig + '</span> </p>');
...
// 当有文件被添加进队列的时候
            task.uploader.on('fileQueued', function (file) {
                $('#fileList').append('<p class="attachment" id="fileId_' + file.id + '"><span class="fileName">' + file.name + '</span><span class="process"></span> <a href="javascript:void(0);" onclick="task.delAttachment(this)" class="delBtn" style="display: none;">删除</a></p>');
//                jQuery('#fileId_'+file.id).find(".delBtn").show();
                jQuery('#fileId_' + file.id).find(".delBtn").click(function () {
...
//                jQuery('#fileId_'+file.id).find(".process").html((percentage*100).toFixed(2)+"%");
                jQuery('#fileId_' + file.id).find(".process").html('<span style="color: red;">上传中...</span>');
            });
//    uploader.on( 'uploadComplete', function( file ) {
//    })
            task.uploader.on('uploadError', function (file) {
                $('#fileId_' + file.id).find('.process').html('<span style="color:red;" >上传出错<span>');
            });

            task.uploader.on('uploadAccept', function (file, response) {
                if (response.code == "S_OK") {    //上传成功
                    $('#fileId_' + file.file.id).find('.process').html('<span style="color:green;" >上传成功<span>');
                    ...
                    jQuery('#fileId_' + file.file.id).find(".process").html("<span style='color: red;'>上传失败</span>");
                }
...
//            data: data,
    ...
    goToTaskDetials: function (type,id,taskId,role) {
        ...
        parent.CC.showMsg('跳转到详情页面', true, false, 'tipsfinish')
        parent.CC.goOutLink('../../workflow/'+jspUrl+'?sid=' + parent.gMain.sid + '&businessId=' +
        businessId+'&taskId='+taskId+'&role='+role, 'type_'+type+'_businessId_' + businessId, '查看详情');
    },
