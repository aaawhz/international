...
var oWrite = {
    ...
    quickSubject: ["紧急！", "会议邀请", "会议纪要", "会议材料"],  // 快捷主题项
    isAutoDestroy: false,
    ...
    initControl: function(){
        var p = this;
        var tipsTemplate = "<div class=\"richinputbox_tips\" style=\"border:solid 1px #ccc; background:#eee; line-height:24px; padding-left:12px;\">还可以输入<span class='red-commend'>{0}</span>个</div>";
        var overflowTips1 = '<div class=\"richinputbox_tips\" style=\"border:solid 1px #ccc; background:#eee; line-height:24px; padding-left:12px;\">收件人数已超过上限<span class="red-commend">{0}</span>人，不能继续添加！</div>';
        var overflowTips2 = '<div class=\"richinputbox_tips\" style=\"border:solid 1px #ccc; background:#eee; line-height:24px; padding-left:12px;\"><span class="red-commend">{1}</span>人未添加，最多添加<span class="red-commend">{0}</span>人</div>';
        ...
        var param = {
            ...
            mailNumError: function(obj){
                ...
                if(obj.createMode == 'paste'){
                    tips = overflowTips2.format(oWrite.toMaxNum, state.fail || '所有粘贴的');
                }
                else{
        ...
        if(bigFileUpload){
            ...
            if(upload_module){
                ...
                var text = '添加最大{0}的附件和文件中转站文件'.format(bigAttachSize);
    ...
    */
    addAutoDesLabelHtml: function (cont) {
        ...
        html += "<hehe <input id=\"chkAutoDestroy\" tabindex=\"21\" type=\"checkbox\" title=\"邮件阅读后自销毁或邮件超过限制天数后自销毁\"><label for=\"chkAutoDestroy\" title=\"邮件阅读后自销毁或邮件超过限制天数后自销毁\">";
        html += " 自销毁邮件";
        html += "</label>";
    ...
    */
    addAutoDesTipsHtml: function (cont) {
        ...
        html += '<li class="pb_5">';
        html += '<label>';
        html += '<input id="ckbAutoDestroy" type="checkbox" class="rm_cb">阅读完成自动销毁';
        html += '</label>';
        html += '</li>';
        html += '<li class="pb_5" style="height:27px;">';
        html += '<label>';
        html += '<input id="ckbDaysDestroy" type="checkbox" class="rm_cb">限制邮件阅读天数';
        html += '</label> <input id="inputReadDays" type="text" value="1" maxlength="30" class="set-txt-b ml_5 mr_5" style="width: 25px;">天 <span id="textReadDays" class="gray ml_20">请输入1-100之间的数值</span></li>';
        html += '<li><p class="gray">1.只在web登录使用的系统内部邮箱生效（包含相同邮箱系统的其它域）。</br>2.自销毁邮件不允许添加附件。</p></li>';
        html += '</ul>';
        html += '<div class="forbiden-bline ta_r">';
        html += '<a id="btnAutoDestroyOk" href="javascript:;" class="xxs-greyBtn">确定</a>';
        html += '<a id="btnAutoDestroyCancel" href="javascript:;" class="xxs-greyBtn ml_5">取消</a>';
        html += '</div>';
        html += '</div>';
    ...
    initEvent : function() {
        ...
        ;(function($){
            ...
            $("#numberEncrypt, #numberEncrypt_label").bind("mouseover", function(){
                $("#txtDes").text("给邮件进行加密，保证邮件全程安全，防止邮件内容泄露。");
            });
            
            //改变文字提示
            $("#numberSign, #numberSign_label").bind("mouseover", function(){
                $("#txtDes").text("给邮件进行签名，以防止他人冒充您的身份。");
            });
           ...
           function undoSign(){
                ...
                var call = function(data){
                        ...
                        if(pass === 1){
                            ...
                            CC.showMsg("验证通过",true, false, "option");
                            ...
                            CC.showMsg( "私钥密码错误",true, false, "error");
                        }else if(pass === 2){
                            CC.showMsg( "密码错误次数达到限制，请于10分钟后重试",true, false, "error");
                        }
            ...
            checkForbidden: function(val){
                ...
                if(p.encryptMail && p.encryptMailClass){
                    if(val == '1'){
                        ...
                        p.encryptMailClass.control.chkbox.attr('title', '您已选择禁止转发，不能同时给邮件设置密码');
                    }
                    else{
                        p.encryptMailClass.control.chkbox.removeAttr('disabled');
                        p.encryptMailClass.control.chkbox.attr('title', '给邮件设置密码');
                    }
                }
            },
            checkEncrypt: function(val){
                if(val){
                    if(GC.check('MAIL_INBOX_FORWARD')){
                        ...
                        chkForbiddenForward.attr('title', '您已选择给邮件设置密码，不能同时再选择禁止转发');
                    }
                }
                else{
                    if(GC.check('MAIL_INBOX_FORWARD')){
                        ...
                        chkForbiddenForward.attr('title', '设置禁止转发该邮件');
                    }
                }
            }
        };
    ...
    bindAutoDestroy: function ($) {
        ...
        $("#btnAutoDestroyOk").click(function (e) {
            ...
            if ($("#ckbAutoDestroy").prop("checked")) {
                ...
                text = "这是一封自销毁邮件，对方收到邮件" + days + "天后，邮件将自动销毁";
            } else {
                $("#chkAutoDestroy").prop("checked", false);
                me.isAutoDestroy = false;
            }
...
//                    mailInfo.hitKeywords = ks.join(',');
    ...
    doSend : function(obj) {
        ...
        if ((subject.match(reg) || content.match(reg))&& jQuery("#attachContainer").find(".zz_comb").length == 0) {
            CC.confirm("您的邮件中提到了附件，您可能忘记添加附件，是否继续发送？", function() {  
    ...
    confirmDoSend : function(obj) {
        if(oWrite.doStatusMessage()) {
              ...
              if (GC.check('MAIL_INBOX_SECURITY')==1 && offSecurity!="true") {
                ...
                if (mailSecLevel < 0)
                {
                    CC.showMsg("请选择邮件密级", true, false, "error");
                    return false;
                }
                var ArraySecLevel = ["公开","秘密","机密"];
                ...
                if (attachFiles && attachFiles.length > 0) {
                    for (var i = 0; i < attachFiles.length; i++){
                        ...
                        for (var j = 0; j < ArraySecLevel.length; j++){
                            ...
                            {
                                CC.showMsg("附件密级标识后才能发送，附件名需包含\"公开\"、\"秘密\"或\"机密\"中的一个", true, false, "error");
                                return false;
                            }
                        ...
                        if (attachSecLevel > mailSecLevel || attachSecLevel == -1) {
                            CC.showMsg("附件密级不能高于邮件密级,请重新标识再发送", true, false, "error");
                            return false;
                        }
                    }
                }
            ...
            if(PageState == PageStateTypes.Uploading) {
                ...
                var ao = {
                    ...
                    title: "系统提示",
                    type: 'div',
                    text: "<div style='padding:15px;'>您的邮件邮件开启了禁止转发选项，含有超大附件的邮件，收件人可以下载并转发。是否继续发送？</div>",
                    changeStyle: false,
                    zindex: 1090,
                    dragStyle: 1,
                    width: 400,
...
//          }
        ...
        function withPassword(){
            if (p.isAutoDestroy) {
                var ao = {
                    ...
                    "type": "text",
                    "width": 460,
                    "text" : "您已经设置了该邮件为自销毁邮件，不允许添加附件，而邮件加密后，将会以附件形式发送，请您取消自销毁设置再发送",
                    "zindex": 20011,
                    "dragStyle": 1,
                    "buttonType": "confirm",
                    "buttons" : [{
    ...
    composeFailCallback: function(resp, url,isDraft) {
        ...
        case "FA_IS_SPAM":
            ...
            CC.alertNoSysClose("您可能没有权限给收件人中的某个用户发信，请检查后发送。", {}, "系统提示");
            return;
            break
    ...
    sensConfirm: function (resp) {
        var me = this,
            ao = {
                ...
                "type": "text",
                "width": 460,
                "text" : "您在办公区，请您使用内网邮箱给域内用户发信，是否清除域内用户？",
                "zindex": 20011,
                "dragStyle": 1,
                "buttonType": "confirm",
                "buttons" : [{
    ...
    **/
    ...
    fillMailData : function() {
        ...
        if(param && typeof (param) == "object") {
            ...
            // 自销毁设置
            var text = "这是一封自销毁邮件，对方阅读完后，邮件将自动销毁";
            if (param.keepFlag == 2) {
                ...
                text = "这是一封自销毁邮件，对方收到邮件" + param.keepDay + "天后，邮件将自动销毁";
            }
...
securityMail.prototype = {
    initiliaze: function(args){
        ...
        if (jQuery("#security_mail_text")[0]) {
            jQuery("#security_mail_text")[0].innerHTML="请选择";
        }
    },
    render: function(){
