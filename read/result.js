...
Home.prototype = {
    ...
    getCalHtml: function (obj) {
        ...
        tmpHtml += '<a href="javascript:void(0)" onClick="parent.MM[parent.gConst.home].goCal()" class="more">'+top.Lang.Mail.Write.cksyrcWnhgJ+'</a>';//<a href="javascript:void(0)" onClick="parent.MM[parent.gConst.home].goCal()" class="more">查看所有日程&gt;&gt;</a>
                ...
                for(var i = 0; i < tempDays.info.length; i++) {
                    ...
                    if (tempStartArray.length == 3 && tempEndArray.length == 3) {
                        ...
                        if (objCurentDate > objStartDate) {
                            startEndTime = (objShowDate.getMonth()-0+1)+top.Lang.Mail.Write.yue+objShowDate.getDate()+top.Lang.Mail.Write.ri+tempData.startTime.toString().substring(0,timeLength-2)+":"+tempData.startTime.toString().substring(timeLength-2,4);                         //月  ||  日 
                        } else {
                            startEndTime = tempStartArray[1]+top.Lang.Mail.Write.yue+tempStartArray[2]+top.Lang.Mail.Write.ri+tempData.startTime.toString().substring(0,timeLength-2)+":"+tempData.startTime.toString().substring(timeLength-2,4);//月  ||  日 
                        }
                        startEndTime += "-"+tempEndArray[1]+top.Lang.Mail.Write.yue+tempEndArray[2]+top.Lang.Mail.Write.ri+tempData.endTime.toString().substring(0,timeLength-2)+":"+tempData.endTime.toString().substring(timeLength-2,4);//月  ||  日 
                    }
        ...
        if (maxCount == 5) {
            ...
            tmpHtml += '<div style="padding: 93px 20px 40px 220px;position: relative;height: 8px;overflow: hidden;margin-right: 20px;">'+top.Lang.Mail.Write.nhmyavCPekhdmsq+'<a href="javascript:void(0)" onClick="parent.MM[parent.gConst.home].goCal()" >'+top.Lang.Mail.Write.chuangjianhuodong+'</a></div>';//<div style="padding: 93px 20px 40px 220px;position: relative;height: 8px;overflow: hidden;margin-right: 20px;">您还没有安排活动，马上去<a href="javascript:void(0)" onClick="parent.MM[parent.gConst.home].goCal()" >创建活动</a></div>
            tmpHtml += '';
            tmpHtml += '</div>';
        }
        return tmpHtml;
    ...
    getCalDay: function (day) {
        ...
        if (oDate.getYear()-nowDate.getYear() == 1) { //跨年度，2103年10月30日
            ...
            return tempArray[1]+top.Lang.Mail.Write.yue+tempArray[2]+top.Lang.Mail.Write.ri;//月  ||  日
        }
        return day;
    },
    /**
    ...
    loadHomeData: function (childDocument) {
        ...
        if (topTipsContainer) {
            ...
            topTipsContainer.innerHTML +='<a style="display:none;" id="custom_home_newMailCount_a" href="javascript:void(0);" onClick="parent.CC.goFolder(1, parent.GE.folderObj.inbox); return false;">'+top.Lang.Mail.Write.shoujianxiang+'<var class="col_orange" id="custom_home_newMailCount_sys1">Loading</var>）</a>';//<a style="display:none;" id="custom_home_newMailCount_a" href="javascript:void(0);" onClick="parent.CC.goFolder(1, parent.GE.folderObj.inbox); return false;">收件箱（<var class="col_orange" id="custom_home_newMailCount_sys1">Loading</var>）</a>
        } else {
            return;
        }
    ...
    replaceTemplate: function(){
                ...
                for(var k in this.datas)
                {
                    ...
                    if( k == "totalSize"  ){
                        ...
                        if( unlimit || this.datas.limitMessageSize == "UNLIMITED") {
                            v =  Lang.Mail.Unlimited || top.Lang.Mail.Write.wuxianrongliang;//无限容量
                        }
        ...
        if(topTipsContainer){
            ...
            topTipsContainer.innerHTML=" <i class='i-symail' style='position: relative;margin-bottom: 3px;'></i><a href='javascript:void(0)' style='font-size: 14px; font-weight:200; margin-right:8px; width:auto;' onclick='parent.MM.home.receiveMail();return false;'>"+top.Lang.Mail.Write.ninyou+"<var id='custom_home_unreadMessageCount'>loading</var>"+top.Lang.Mail.Write.fengxinyoujian+"</a>";// <i class='i-symail' style='position: relative;margin-bottom: 3px;'></i><a href='javascript:void(0)' style='font-size: 14px; font-weight:200; margin-right:8px; width:auto;' onclick='parent.MM.home.receiveMail();return false;'>您有<var id='custom_home_unreadMessageCount'>loading</var>封新邮件</a>
            var folders=CM.folderMain[gConst.dataName];
            var v=0;
            for (var z = 0; z < folders.length; z++) {
                var cur = folders[z];
            ...
            if(num>2){
                topTipsContainer.innerHTML +=html1+"<a href='javascript:void(0)' class='showhide' style='color:#999999; font-size: 10px; font-weight:100; margin-left:-12px;' id='ssd' onclick='parent.MM.home.showhide(this);return false;'>"+top.Lang.Mail.Write.xianshigengduo+"</a><span style='word-wrap:break-word; display:none'>   "+html+"</span>";//<a href='javascript:void(0)' class='showhide' style='color:#999999; font-size: 10px; font-weight:100; margin-left:-12px;' id='ssd' onclick='parent.MM.home.showhide(this);return false;'>显示更多&gt;&gt;</a><span style='word-wrap:break-word; display:none'>   
            }
            else{
    ...
    showhide:function(_this){
        ...
        if(anextspan=="inline"){
            _this.innerHTML=top.Lang.Mail.Write.xianshigengduo;//显示更多&gt;&gt;
            ...
            jQuery("#ifrm_home").contents().find(".gwel-info p > .showhide").css("color","#CCC").html(top.Lang.Mail.Write.xianshigengduo);//显示更多&gt;&gt;
            _this.innerHTML=top.Lang.Mail.Write.yincang;//隐藏&lt;&lt;
    ...
    loadedCallback: function(){
        ...
        if(usedPercent > 80 && usedPercent < 100){
            volume = '<div>'+top.Lang.Mail.Write.txndyzdqTgddsxq+'<span id="clear" class="c_ff6600 pl_5 pr_5">'+top.Lang.Mail.Write.qingliyoujian+'</span>'+top.Lang.Mail.Write.huo;//<div>提醒：您的邮箱容量即将达到上限{0}，请<span id="clear" class="c_ff6600 pl_5 pr_5">清理邮件</span>或
            volume+= '<span id="contact" class="c_ff6600 pl_5 pr_5">'+top.Lang.Mail.Write.lxglysjrlBGMHM+'</span>。<a id="closeVolume2" onclick="parent.MM[parent.gConst.home].closeVolumeTips()" href="javascript:void(0)">'+top.Lang.Mail.Write.guanbitixing+'</a></div>';//<span id="contact" class="c_ff6600 pl_5 pr_5">联系管理员升级容量</span>。<a id="closeVolume2" onclick="parent.MM[parent.gConst.home].closeVolumeTips()" href="javascript:void(0)">关闭提醒</a></div>
            needTips = true;
        }
        else if(usedPercent >= 100){
            volume = '<div>'+top.Lang.Mail.Write.txndyHbbvbddsxq+'<span id="clear" class="c_ff6600 pl_5 pr_5">'+top.Lang.Mail.Write.qingliyoujian+'</span>'+top.Lang.Mail.Write.huo;//<div>提醒：您的邮箱容量已满，已达到上限{0}，请<span id="clear" class="c_ff6600 pl_5 pr_5">清理邮件</span>或
            volume+= '<span id="contact" class="c_ff6600 pl_5 pr_5">'+top.Lang.Mail.Write.lxglysjrlsCnAs+'</span>。<a id="closeVolume2" onclick="parent.MM[parent.gConst.home].closeVolumeTips()" href="javascript:void(0)">'+top.Lang.Mail.Write.guanbitixing+'</a></div>';//<span id="contact" class="c_ff6600 pl_5 pr_5">联系管理员升级容量</span>。<a id="closeVolume2" onclick="parent.MM[parent.gConst.home].closeVolumeTips()" href="javascript:void(0)">关闭提醒</a></div>
            needTips = true;
        }
