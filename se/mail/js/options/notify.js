...
//  
    ...
    html.push("<label for=\"\">"+top.Lang.Mail.Write.yichangshifasong+"</label>");//<label for=\"\">异常时发送</label>
    //html.push("<label for=\"\">"+Lang.Mail.ConfigJs.set_sessionMode_exception+"</label>");
    html.push('<span style="display: inline-block;  color: #828282;  font-weight: normal;  padding-left: 10px;">'+top.Lang.Mail.Write.zqtcsbsnjcWuJqfQEhfsdltxdxgn+'</span>');//<span style="display: inline-block;  color: #828282;  font-weight: normal;  padding-left: 10px;">在其他城市(不是您经常登录的城市，包括海外)登录时，会发送登录提醒短信给您。</span>
    html.push("</div>");
    html.push("</li>");
    ...
    html.push("<label for=\"\">"+top.Lang.Mail.Write.meicidufasong+"</label>");//<label for=\"\">每次都发送</label>
    ...
    html.push("<label for=\"\">"+top.Lang.Mail.Write.bufasong+"</label>");//<label for=\"\">不发送</label>
    html.push("</div>");
    html.push("</li>");
    //上次登陆时间和IP,首次登陆隐藏该信息
    if (lastLoginTime!="") {
        var oDate = new Date(lastLoginTime.replace(/-/g,"/"));
        var day = new Array(top.Lang.Mail.Write.xingqiri, top.Lang.Mail.Write.xingqiyi, top.Lang.Mail.Write.xingqier, top.Lang.Mail.Write.xingqisan, top.Lang.Mail.Write.xingqisi, top.Lang.Mail.Write.xingqiwu, top.Lang.Mail.Write.xingqiliu)[oDate.getDay()];//星期日  ||  星期一  ||  星期二  ||  星期三  ||  星期四  ||  星期五  ||  星期六
        
        html.push("</li>");
        html.push("<li class=\"pt_15\" style=\"padding-left: 60px;\">");
        html.push("<div>");
        if (lastLoginTime!=""&&lastLoginTime!="") {
            html.push(""+top.Lang.Mail.Write.shangcidenglushijian+"<label style=\"padding-right: 200px;\">"+lastLoginTime+" ("+day+")</label>");//上次登录时间：<label style=\"padding-right: 200px;\">
        }
        if(lastLoginIp!=""&&lastLoginCity!="") {
            html.push(""+top.Lang.Mail.Write.shangcidenglu+"<label style=\"padding-right: 20px;\">"+lastLoginIp+"("+lastLoginCity+")</label>");   //上次登录IP：<label style=\"padding-right: 20px;\">
        }
        
        html.push('<a onclick="loginNotify.goLoginLog();" href="javascript:;">'+top.Lang.Mail.Write.denglujiluchaxun+'</a>');//<a onclick="loginNotify.goLoginLog();" href="javascript:;">登录记录查询</a>
        html.push("</div>");
        html.push("</li>");
    }
    html.push("</ul>");
...
loginNotify.goLoginLog = function() {
    CC.goLogLink(top.Lang.Mail.Write.faxinrizhi,"login");//发信日志
};
