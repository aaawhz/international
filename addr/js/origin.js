...
(function (window) {
    ...
    Friend.prototype = {
        ...
        exchange: function (obj) {
            ...
            user.sexual = user.sex == 1 ? '男' : (user.sex == 2 ? '女' : '保密');
...
//            var dept = t.getDept(user.dept_id[0]);
    ...
    ComUserInfo.prototype = {
        exchange: function (user) {
            ...
            user.sex = user.sex == 0 ? '男' : user.sex == '1' ? '女' : '保密';
            ...
            user.userDateType = user['user_date_type'] ? '入学时间' : '入职时间';
            user.userNumber = util.encodeHTML(user['user_number']) || '';
            user.userNumberType = user['user_number_type'] ? '学号' : '工号';
...
//            options.list = options.list || t.contact;
        ...
        bindDrag: function (container) {
            ...
            function setHelper(helper) {
                ...
                var html = util.format('<div class="gragcon_bd"><i class="i-move mr_5"></i><span class="grag_txt">{0}</span></div>', util.format("正拖动{0}个联系人", count));
                helper.html(html);
            }
        },
