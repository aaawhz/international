...
SWFObject.prototype.getHTML = function () {
    ...
    if (navigator.plugins && navigator.mimeTypes && navigator.mimeTypes.length) {
        ...
        con += '<embed class="flash" type="application/x-shockwave-flash"  pluginspage="' + protocol + '//www.macromedia.com/go/getflashplayer" src="' + this.getAttribute('swf') + '" width="' + this.getAttribute('width') + '" height="' + this.getAttribute('height') + '"';
        ...
        con = '<object class="flash" id="' + this.getAttribute('id') + '" classid="' + this.getAttribute('classid') + '"  codebase="' + protocol + '//download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=' + this.setAttribute("version") + ',0,0,0" width="' + this.getAttribute('width') + '" height="' + this.getAttribute('height') + '">';
...
var upload_template = {
    frame: ['{closeBtn}<i class="big_{fileIconClass} fl"></i>',
        '<div class="sk_lt">',
        '    <div class="clearfix sk_cont">',
        '        <div class="fl fup_name">{prefix}.{suffix}</div>',
        '        {action}',
        '    </div>',
        '    <div class="clearfix">',
        '        {state}',
        '        <span id="{taskId}_size" class="fr">{fileSizeText}</span>',
        '    </div>',
        '</div>'].join(''),
    close: '<a style="top:5px; right:5px;" class="i-fakeTxt-close" href="javascript:fGoto();" id="" taskid="{taskId}" fileid="{fileId}" uploadType="{uploadType}" fileType="{fileType}" command="DeleteFile"></a>',
    process: [
        '<div class="prisdr-progress-bar fl" style="width:98px; overflow:hidden;">',
...
var attach = {
    ...
    composeId: "",
    data: [],
    maxSize: (parent.gMain.attachMaxSize - 0) || 50, //邮件&附件最大限制 单位为M
    ...
    checkAttach: function (file) {
        ...
        //在小工具下 file.fileName 返回的是文件路径
        if(/[\\\\:]/g.test(name) ){
            filename = parent.Util.getFileNameByPath(name);

            if( treg.test(filename)  ){
            
                hasErrorName = true;
            }

        } else if( treg.test(name) ){
        ...
        if( hasErrorName ){
             ...
             if (p.delegateFun['onFormatError'] && (typeof p.delegateFun['onFormatError'] == 'function')){
                 ...
                 p.delegateFun['onFormatError'] && p.delegateFun['onFormatError'](name, top.Lang.Mail.Write.wjmbhdtszf);//文件名包含:\\\/*\?\"\'\<\>等特殊字符
                 return false;
             }
    ...
    btnAddAttachFromDisk: function () {
        var d = new parent.DiskDialog();
        d.create(attach.tempDiskCall);

        return;
        var p = this;
        var composeid = oWrite.composeId;
        var obj = {
            type: 1,
    ...
    uploadOk: function (type, taskId, data) {
        if (type == "form") {
            ...
            this.attachList.push({type: attachType, taskId: taskId, fileId: data.fileId, fileSize: data.fileSize, fileName: data.fileName});
        }
    ...
    removeFile: function (id, size) {
        ...
        if (typeof(id) == "string" || typeof(id) === "number") {
            ...
            for (j = 0; j < this.data.length; j++) {
                if (this.data[j].fileId == id || this.data[j].taskId == id) {
    ...
    deleteTasks: function (fids, funcDel) {
        //var data = [];
        //data.push({func: parent.gConst.func.deleteAttach,'var': {composeId:oWrite.id,items:fids}});
        //data.push({func: parent.gConst.func.refreshAttach,'var':{id:oWrite.id}});
        ...
        var attachCall = function (resp) {
            try {
                if (resp.code == "S_OK") {
                    //oWrite.showAttachContainer(resp["var"]);
    ...
    addAttachContainer: function (obj) {
        ...
        li.title = fn;
        li.id = "li_" + taskId;
        li.className = 'clearfix zz_comb fup';//'upLoad shadow';
        ...
        oWrite.setSubjectTxt(subject, true);
    },
    ...
    addDiskFiles: function (obj, isAdd) {
        ...
        var df = parent.GE.diskFiles[oWrite.composeId];
        var beforeIndex = 0;
        if (isAdd) {
            beforeIndex = df.length;
            parent.GE.diskFiles[oWrite.composeId] = parent.GE.diskFiles[oWrite.composeId].concat(obj);
        }
        if (obj) {
            for (var i = 0; i < obj.length; i++) {
                var file = obj[i];
                var id = file.id;
                var fn = subject = file.name.encodeHTML();
                var fen = fn.substr(fn.lastIndexOf('.') + 1);
                var fileIconClass = parent.Util.getFileClasssName(fn);
                fn = fn.substr(0, fn.length - fen.length - 1);
                var sfn = this.getFileShortName(fn);
                ...
                var html = upload_template.frame;
                var div = document.createElement("div");
                div.title = fn;
                div.id = "FromDiskFiles_" + id + "_" + (beforeIndex + i);
                div.className = 'clearfix zz_comb fup bigAttach';//'upLoad shadow';
//                html.push("<div class=\"upLoadBg\">");
...
//                html.push("</div>");
                ...
                html = Utils.format(html, {
                    'closeBtn': upload_template.removeDiskFile,
                    'state': upload_template.success,
                    'prefix': '{prefix}',
                    'suffix': '{suffix}',
                ...
                var data = {
                    ...
                    suffix: fen,
                    fileSizeText: size,
                    fileId: id.toString().trim(),
                    fileType: 'disk',
                    index: beforeIndex + i
                };
                div.innerHTML = Utils.format(html, data);
    ...
    showAttachContainer: function (files, id) {
        ...
        try {
            for (i = 0; i < files.length; i++) {
                ...
                file.fileName  = file.fileName.encodeHTML();
                this.addAttachContainer(file);
            }
        } catch (e) {
        }
        this.setAttachHeight();
    ...
    setAttachHeight: function () {
        ...
        if (lis.length > 0) {
            parent.El.removeClass(ul, 'hide');
            jQuery(ul).show();
            jQuery(ul).show();
        } else {
            parent.El.addClass(ul, 'hide');
            utool.hideBigAttachTransformTips();
        }
...
var UploadActivex = {
    ...
    isSetup: function () {
        ...
        try {
            var obj = new ActiveXObject("ydRIMailToolAutoUpdateCtrl.AutoUpdate.1");
            if (obj == null) {
                return false;
            }
            var version = obj.GetActiveXVersion("ydCxdndctrl.Upload.1");
            return true;
        } catch (e) {
        }
        if (!setup) {
    ...
    var upload_module = {
        ...
        init: function (model) {
            ...
            if (t.isSupportMultiThreadUpload) {
                ...
                utool.registerBigAttachTipsEvent();
            }
            else {
                ...
                if (window.VBArray) {
                    jQuery(utool.autoUploadObjectString).appendTo(jQuery(document.body));
                }

            }

            function registerNormalUpload() {
        ...
        deletePreuploadFile: function (fileName) {
            ...
            RequestBuilder.call("upload:deleteTasks", requestXml, function (result) {
                if (result["code"] == "S_OK") {
                    top.Debug.write(top.Lang.Mail.Write.scbcscdfj + fileName);//删除本次上传的附件:
...
JSForFlashUpload = {
    ...
    oncomplete: function (taskId, responseText) {
        ...
        if (result.success) {
            ...
            file.state = "error";
            file.updateUI();
            CC.alert(top.Lang.Mail.Write.wjscsschzs);//文件上传失败，请删除后重试！
        }
        uploadManager.autoUpload();
    },
    /**
...
//机械化操作
utool = {
    autoUploadObjectString: "<object id=\"autoUploadControl\" height=\"1\" width=\"1\" classid=\"clsid:63A691E7-E028-4254-8BD5-BDFDB8EF6E66\" codebase=\"" + parent.CC.getResourceAbsoluteURL() + "/mail/activex/RIMSetup.CAB#version=1,0,0,0\"></object>",
    getMaxUploadSize: function () {
        return attach.maxSize;
    },
    getMaxUploadSizeWithUnit: function () {
        var s = utool.getMaxUploadSize();
        var r = '';
        if (s > 0 && s < 1) {
            r = (s * 1024) + 'K';
        }
    ...
    getUploadTipMessage: function () {
        var tip = Utils.format(top.Lang.Mail.Write.tjxydfj, [utool.getMaxUploadSizeWithUnit()]);//添加小于{0}的附件
        var attachLink = jQuery('#aattach');
        if (supportUploadType.isSupportScreenShotUpload) {
            //tip += "，可使用 Ctrl+V 粘贴附件和图片";
            if (parent.GC.check("DISK")) {
                var bigAttachSize = parent.gMain.tempUploadMaxFileSize || 0;
                if (bigAttachSize) {
                    bigAttachSize = parent.Util.formatSize(parent.Util.str2Num(bigAttachSize), null, 2);
                }
                ...
                tip = top.Lang.Mail.Write.tjzddfj.format(bigAttachSize);//添加最大{0}的附件
            }
            else {
                bigAttachSize = '1G';
            }
            tip = top.Lang.Mail.Write.tjzddtscfj.format(bigAttachSize);//添加最大{0}的附件<br/>可使用Ctrl+V粘贴上传附件
        } else {
            if (navigator.platform.indexOf("Mac") === -1) {
                tip += top.Lang.Mail.Write.anzhuang + parent.gMain.frmPath + "/activex/MailThinkmail_Tool.exe'>"+top.Lang.Mail.Write.yxxgj+"</a>"+top.Lang.Mail.Write.jkztscfj
    ...
    registerBigAttachTipsEvent: function () {
        var bigFileUpload = jQuery('#aattach');
        ...
        if (upload_module) {
            ...
            var text = top.Lang.Mail.Write.tjzddtscfj.format(bigAttachSize);//添加最大{0}的附件<br/>可使用Ctrl+V粘贴上传附件
    ...
    hideToolTips: function () {
        jQuery('#bigFileUploadTips1').hide();
    },
    //弹出禁用flash上传的对话框
    showDisableFlashMsg: function () {
        if (confirm(top.Lang.Mail.Write.nsfyjysczj)) {//您是否要禁用Flash上传组件?
            var d = new Date();
            d.setFullYear(2099);
            $Cookie.set({name: 'flashUploadDisabled', value: '1', expries: d});
            alert(top.Lang.Mail.Write.sczjydscfs);//Flash上传组件已经禁用，您下次打开写信页将使用原始但是稳定的上传方式。
        }
    },
    //检测加上文件大小是否超标
    ...
    getBlockUploadUrl: function () {
        var url = window.location.protocol + '//' + window.location.host + "/RmWeb/mail?func=attach:upload2&sid=" + upload_module.model.sid + "&composeId=" + upload_module.model.composeId;
        return url;
    },
    /*
    ...
    checkUploadResultWithResponseText: function (param) {
        ...
        var result = {};
        var ao = attach.getFilesByString(text) || {};
        if (ao.code == "S_OK") {
            result = ao;
            result.success = true;
    ...
    showUploadTip: function () {
        ...
        if (j.length == 0) {
            if (!jQuery.browser.safari) {
                j = $("<div id='divUploadTip' style='position:absolute;display:none'>"+top.Lang.Mail.Write.azxgj+"</div>").appendTo(document.body);//<div id='divUploadTip' style='position:absolute;display:none'>安装小工具</div>
            }
        }
    ...
    showBigAttachTransformTips: function () {
        ...
        var tips_ui = jQuery('<div id="bigAttachTransform_tips" class="yellowTips p_relative" style=""><i id="bigAttachTransformCloseBtn" style="right:2px; top:2px;" class="i-fakeTxt-close"></i>'+top.Lang.Mail.Write.fjcgjjwfxz+'</div>'.format(attach.maxSize + 'M'));//<div id="bigAttachTransform_tips" class="yellowTips p_relative" style=""><i id="bigAttachTransformCloseBtn" style="right:2px; top:2px;" class="i-fakeTxt-close"></i>附件超过{0}, 将作为超大附件上传, 保存在文件中转站, 15天内有效, 若被删除或过期, 收件人将无法下载.</div>
        var container = jQuery('.edi_container');

        container.prepend(tips_ui);
        jQuery('#bigAttachTransform_tips').show();

        jQuery('#bigAttachTransformCloseBtn').unbind('click').bind('click', function () {
            jQuery('#bigAttachTransform_tips').hide();
        });
    },
    hideBigAttachTransformTips: function () {
        if (jQuery('#bigAttachTransform_tips')) {
            jQuery('#bigAttachTransform_tips').hide().remove();
        }
    },
    mergeBigAttachList: function (list) {
        if (!parent.GE.diskFiles[oWrite.composeId]) {
            parent.GE.diskFiles[oWrite.composeId] = [];
        }
        var dList = parent.GE.diskFiles[oWrite.composeId];
        var dMap = {};
        var tList = [];
        for (var i = 0; i < dList.length; i++) {
            if (dList[i].url) {
                dMap[dList[i].url] = true;
            }
        }
        for (var i = 0; i < list.length; i++) {
            if (list[i].url && !dMap.hasOwnProperty(list[i].url)) {
                tList.push(list[i]);
            }
        }
        if (tList.length) {
            parent.GE.diskFiles[oWrite.composeId] = parent.GE.diskFiles[oWrite.composeId].concat(tList);
        }
    },
    deleteBigAttachById: function (id) {
        var dList = parent.GE.diskFiles[oWrite.composeId]
        if (id && dList) {
            for (var i = 0; i < dList.length; i++) {
                if (dList[i].url && dList[i].id && dList[i].id == id) {
                    parent.GE.diskFiles[oWrite.composeId] = parent.GE.diskFiles[oWrite.composeId].splice(i, 1);
                }
            }
        }
    }
...
UploadManager.prototype = {
    ...
    uploadFile: function (param) {
        ...
        for (var i = 0; i < param.length; i++) {
            ...
            //校验附件
            if (item.uploadType != 'screenShot') {
                if (!attach.checkAttach(item)) {
                    return false;
                }
            }
            ...
            if (!isMultiThread && (item.uploadType == "multiThread" || jQuery.browser.mozilla)) {
                isMultiThread = true;
            }
            if (item.fileSize) {
            ...
            item.name = item.fileName.substr(item.fileName.lastIndexOf('\\') + 1);
        }

        if (!utool.checkSizeSafe(totalSize)) {
            if (parent.GC.check('MAIL_VAS_HUB')) {
                if (isMultiThread) {
                    if (jQuery.browser.mozilla) {
                        var obj = {
                            ...
                            buttons: [
                                {
                                    text: top.Lang.Mail.Write.sycdfjsc,//使用超大附件上传
                                    clickEvent: function () {
                        ...
                        changeToBigFileUpload(param);
                    }
                }
                else {
                    var info = ['<div class="pop-wrap">',
                        '<div class="j_tool_w">',
                        '    <p class="j_tool_note">'+top.Lang.Mail.Write.sccdfxgjcj+'</p>',//    <p class="j_tool_note">上传超大附件, 需要先安装企邮小工具插件.</p>
                        '    <div class="j_tool_b">',
                        '        <p>'+top.Lang.Mail.Write.azhnkyyxgn+'</p>',//        <p>安装后：您可以使用以下功能：</p>
                        ...
                        '            </li>',
                        '            <li>',
                        '                <i class="i-zt mr_10"></i><span>'+top.Lang.Mail.Write.zttdzw+'<br>'+top.Lang.Mail.Write.ztfsscfj+'</span>',//                <i class="i-zt mr_10"></i><span>粘贴图到正文<br>粘贴方式上传附件</span>
                        '            </li>',
                        '                <li style="">',
                    ...
                    var obj = {
                        ...
                        width: "480",
                        text: info,
                        title: top.Lang.Mail.Write.qyxgjazts,//企邮小工具安装提示
                        buttons: [
                            {
    ...
    removeFile: function (file) {
        ...
        for (var i = 0; i < list.length; i++) {
            ...
            if (f.taskId == file.taskId) {
                ...
                attach.removeFile(file.taskId, file.fileSize);
                break;
            }
        }
    ...
    render: function (param) {
        ...
        var list = this.fileList;
        if (param && param.type == "refresh") {
            container.find('.fup').each(function () {
                if (!jQuery(this).hasClass('bigAttach')) {
                    jQuery(this).remove();
                }
            });
        }
...
var ComposeMessages = {
    PleaseUploadSoon: top.Lang.Mail.Write.fjsczjxdfj,//附件上传中，请稍后再添加新的附件
    FileSizeOverFlow: '<div class="pop-wrap"><div class="j_tool_w"><p style="padding:0 10px"><strong>'+top.Lang.Mail.Write.ntjdwjdxcg+'</strong></p><p class="j_tool_note"><font style="color:#333;">'+top.Lang.Mail.Write.bydxnfjlfs+'</font><br/>'+top.Lang.Mail.Write.cdfjbygsxz+'</p></div></div>',//"对不起，文件大小超出附件容量限制。"//<div class="pop-wrap"><div class="j_tool_w"><p style="padding:0 10px"><strong>您添加的文件大小超过{0}</strong></p><p class="j_tool_note"><font style="color:#333;">不用担心 您可以使用超大附件来发送。</font><br/>超大附件保存在文件中转站中，更环保，对方也可以高速下载.</p></div></div>
    FileNameExist: top.Lang.Mail.Write.xtdwjmhzsc,//"{0}"相同的文件名已经存在,请重命名后再上传
    FileUploadError: top.Lang.Mail.Write.wjscsb,//文件\"{0}'上传失败
    FileIsUsing: top.Lang.Mail.Write.dbqwjwjhzs,//对不起，文件正在被其它应用程序占用，请关闭文件后再试。
    BrowserDonotSuppertScreenShot: top.Lang.Mail.Write.ndllqcjpgn,//您的浏览器暂不支持截屏功能
    GetLargeAttachFail: top.Lang.Mail.Write.hqdfjqshzs,//获取大附件失败，请稍后再试
    ExistFileName: top.Lang.Mail.Write.yscfjmmhzs,//已上传附件中存在相同文件名，请重命名后再试。
    NoFileSize: top.Lang.Mail.Write.wjdxwjwfsc
};//"{0}"文件大小为 0 字节，无法上传
...
//    },
    ...
    isSupport: function () {
        ...
        if (document.all && (window.ActiveXObject || /rv:11.0/.test(navigator.userAgent))) {
            //用户曾经手动触发禁用flash上传
            /*
...
var upload_module_ajax = {
    ...
    registerDocEvent: function (doc) {
        if(document.documentMode!=9){
            ...
            doc.body.addEventListener("dragleave", _dragleave, false);
        }
    ...
    isSupport: function () {
        ...
        if (window.FormData && /safari|Chrome|Firefox|MSIE 1\d|rv:1\d/i.test(navigator.userAgent)) {
            return true;
        } else {
            return false;
        }
            ...
            if (xhr.readyState == 4 && xhr.responseText) {
                ...
                if (uploadResult.success) {
                    ...
                    if (response.indexOf('FA_ATTACH_SIZE_EXCEED') > 0) {
                        ...
                        CC.alert(top.Lang.Mail.Write.dbqscschzs);//对不起，上传附件失败，请删除后重试！
                    }
                }
                uploadManager.autoUpload();
            }
...
function _dragenter(e) {
    var html,
        box = jQuery(".editorWrap");
    if (!upload_module.model.composeId || !e.dataTransfer.files || e.dataTransfer.files.length == 0) {
        return;
    }    
    if (box.find(".R0_in").length < 1) {
        html = "<div class=\"R0\"><div class=\"R0_in\">"+top.Lang.Mail.Write.qbwjtwfjtj+"</div></div>";//<div class=\"R0\"><div class=\"R0_in\">请把文件拖放到这个区域，就可作为附件添加。</div></div>
        box.append(html);
    } else {
        box.find(".R0").show();
        if (drapTimer) {
...
function _dragleave(e) {
    drapTimer = setTimeout(function () {
        jQuery(".editorWrap").find(".R0").hide();
    }, 50);
    e.preventDefault();
}
function _drop(e) {
    ...
    for (var i = 0; i < files.length; i++) {
        if (files[i].fileSize == 0) {
            var errorMsg = top.Lang.Mail.Write.bntfwwldwj;//不能拖放文件夹，以及大小为零的文件
            CC.alert(errorMsg);
            return;
        }
    }
...
UploadFileItem.prototype = {
        ...
        if (isUpdateProgress) {
            ...
            switch (this.state) {
                ...
                case "complete":
                    ...
                    oWrite.setSubjectTxt(this.fileName, true);
                    PageState = PageStateTypes.Common;
                    break;
                case "uploading":
                ...
                case "blockerror": //上传中断
                    ...
                    break;
                case "error":
                    htmlCode = this.container.innerHTML;
                    break;
            }
            this.container.className = 'clearfix zz_comb fup'; //"upLoad shadow";
    ...
    updateInsertImageLoading: function () {
        switch (this.state) {
            case "waiting":
            case "uploading":
                CC.showMsg(top.Lang.Mail.Write.tpjzz);//图片加载中...
                break;
            case "complete":
                CC.hideMsg();
                break;
...
//                        </div>';
        ...
        var htmlCode = this.template.frame;
        htmlCode = Utils.format(htmlCode, {
            'closeBtn': this.template.close.replace('DeleteFile', 'RemoveFile'),
            'state': this.template.waiting,
            'prefix': '{prefix}',
            'suffix': '{suffix}',
            'action': '',
            'fileSizeText': '{fileSizeText}',
...
//                        </div>';
        ...
        var htmlCode = this.template.frame;
        htmlCode = Utils.format(htmlCode, {
            'closeBtn': this.template.close,
            'state': this.template.success,
            'action': '',
            'prefix': '{prefix}',
            'suffix': '{suffix}',
            'fileSizeText': '{fileSizeText}',
...
//            htmlCode += "&nbsp;|&nbsp;<a href=\"javascript:void(0)\" command=\"ResumeUpload\" taskid=\"{taskId}\">续传</a>"
            action.unshift(this.template['continue']);
        }
...
//        old1.innerHTML = new1.innerHTML;
        $(this.container).html(this.getProgressUploadingHTML());
    },
    upload: function () {
        PageState = PageStateTypes.Uploading;
...
var upload_module_multiThread = {
    ...
    init: function (isDocumentClosed) {
        ...
        window.captureClipboard = function () {
            ...
            return true;    //2017Q1:小工具的粘贴有问题，在编辑器ueditor实现ctrl+v粘贴，故这里先屏蔽了，直接return
...
MultiThreadUpload = {
    create: function (isDocumentClosed) {
        ...
        if (jQuery.browser.msie || /rv:11.0/.test(navigator.userAgent)) {
                ...
                if (obj.status < 4) {
                    ...
                    if (obj.status == 4) {
                        if (obj.stopReason == 1) {
                            ...
                            // 完成上传了
                            attach.uploadOk('ActiveX', obj.taskId, obj);
                    ...
                    if (obj.stopReason > 2) {
                        ...
                        try {
                            ...
                            errorLog = top.Lang.Mail.Write.scsbqzs; //errorLog.stopReason;//上传失败，请重试
...
function commonAttachFrameOnLoad(frmAttach, isInserImage) {
    ...
    try {
        ...
        if (obj && obj.code == "S_OK") {
            ...
            if (upload_module.model.autoSendMail) oWrite.doSend();
//            if(isInserImage){
...
//            }
            ...
            CC.confirm(
                top.Lang.Mail.Write.fjscsbqzs,//附件上传失败，请重试。
                function () {
                    form.submit();
                },
...
function refreshAttach(onlyRefreshAttach) {
    ...
    if (upload_module.model.composeId) {
        var obj = {
            ...
            msg: top.Lang.Mail.Write.sxfjsb
        };//刷新附件失败
        parent.MM.mailRequestApi(obj);
    }
    attach.setAttachHeight();
...
//var aBigAttachUploadManager = [];
//// 启用大附件上传接管小工具普通附件上传
//function changeToBigFileUpload(fileList){
//    var len = aBigAttachUploadManager.length;
//    //启用新视图模板
//    if(UploadView && UploadModel){
//        var view = new UploadView();
//        var model = new UploadModel();
//        view.containerTag = '.fup';
//        view.itemTemplete = ['<div clientTaskno="{clientTaskno}">',
//            '<div class="clearfix zz_comb fup bigAttach">',
//            '<i class="big_{expandClassName} fl" style="height:32px;"></i>',
//            '<div class="sk_lt">',
//            '    <div class="clearfix sk_cont">',
//            '        <div class="fl fup_name" style="">{fileName}.{expandName}</div>',
//            '        <input type="hidden" fname="{fileName}" exname="{expandName}" value="{fileName}"/>',
//            '        {delbtnTemplete}',
//            '        <span class="btnwrap" style="margin-left:15px;">{handleBtnTemplete}</span>',
//            '    </div>',
//            '    <div class="clearfix">',
...
//            '</div>',
//            '</div>'].join('');
//        view.progressBarTemplete = [
//            '<div class="prisdr-progress-bar" style="width:98px; *zoom:1;">',
//            '    <div class="prisdr-progress"></div>',
//            '    <div class="prisdr-progress-cur ta_c" style=" line-height:10px;width:{processText}; position:inherit;"><var class="processText" style=" display:inline-block; line-height:11px; vertical-align:2px;*vertical-align:4px;color:#fff;font-size:11px; -webkit-text-size-adjust:none; -webkit-transform:scale(0.875);">{processText}</var></div>',
//            '</div>'
//            ].join('');
//        view.delTemplete = '<a style="top:5px; right:5px;" class="i_t_close delbtn" href="javascript:fGoto();" id="" clientTaskNo="{clientTaskno}"></a>';
//        view.md5LoadingTemplete = '<div class="statuswrap" style="float:left;"><span class="gray">正在扫描本地文件</span></div>';
//        view.fileSizeTemplete = '<span class="fileSizeText" style="float:right">{fileSizeText}</span>';
//        view.waitTemplete = '<div class="statuswrap" style="float:left;"><span class="gray">等待上传</span></div>';
//
//        // 启用新委托回调
//        var oldViewComplete = view.complete;
//        var oldViewRender = view.renderList;
//        view.complete = function(options){
//            oldViewComplete.call(this, options);
//            PageState = PageStateTypes.Common;
//            try{
//                var obj = [], sucList = this.model.fileUploadSuc;
//                if(sucList && sucList.length){
//                    for(var i=0;i<sucList.length;i++){
//                        obj.push({
//                            id: '',
//                            name: sucList[i].name,
//                            size: sucList[i].size,
//                            url: sucList[i].shareUrl,
//                            shareTime: sucList[i].validateTime
//                        });
//                    }
//                }
//                if(!parent.GE.diskFiles[oWrite.composeId]){
...
//            attach.setAttachHeight();
//        };
//        view.renderList = function(options){
//            PageState = PageStateTypes.Uploading;
//            oldViewRender.call(this, options);
//            attach.setAttachHeight();
//        };
//
//        // 重写insertEle方法
//        view.insertEle = function(ele){
//            var container = this.getContainer();
//            container.append(ele);
//        }
//
//        aBigAttachUploadManager[len]=new UploadControler({
//            model           : model,
//            view            : view,
//            perSize         : 1024*1024*1024,
//            btnUploadId     : 'aattach_x',
//            listContainerid : 'attachContainer',
//            deleteCall: function(options){
//                PageState = PageStateTypes.Common;
//                attach.setAttachHeight();
//            }
//        });
//        aBigAttachUploadManager[len].trigger('select',{fileList:fileList});
//        utool.showBigAttachTransformTips();
//    }
//}
...
function changeToBigFileUpload(fileList) {
    if (bigUpload) {
        bigUpload.app.selectCall(fileList);
    }
    else {
        bigUpload = controlUpload.init({
            containerId: 'attachContainer',
            containerClass: 'clearfix zz_comb fup bigAttach',
            fastMaxFileSzie: parent.gMain.tempUploadMaxFileSize || 1024 * 1024 * 1024,
            itemTag: 'div',
            filelength: 80
        });
        bigUpload.on('afterRender', function () {
            PageState = PageStateTypes.Uploading;
            attach.setAttachHeight();
        });
        bigUpload.on('afterComplete', function () {
            PageState = PageStateTypes.Common;
            var attachList = bigUpload.getUploadSuccessList();
            var list = [];
            for (var i = 0; i < attachList.length; i++) {
                list.push({
                    id: attachList[i].clientTaskno || '',
                    name: attachList[i].name,
                    size: attachList[i].size,
                    url: attachList[i].shareUrl,
                    shareTime: attachList[i].validateTime
                });
            }
            utool.mergeBigAttachList(list);
            attach.setAttachHeight();
        });
        bigUpload.on('afterDelete', function (id) {
            PageState = PageStateTypes.Common;
            attach.setAttachHeight();
            utool.deleteBigAttachById(id);
        });
        bigUpload.app.selectCall(fileList);
    }
    utool.showBigAttachTransformTips();
