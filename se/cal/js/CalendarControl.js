...
Calendar.prototype.getStyle = function(){
    ...
    str += '.calendar{position:absolute;width:150px!important;width /**/:142px;height:184px!important;height /**/:174px;background-color:' + this.style.bodyBackColor + ';border:1px solid ' + this.style.borderColor + ';left:0px;top:0px;z-index:9999;}\n';
    str += '.cdrHeader{background-color:' + this.style.headerBackColor + ';width:150px;height:22px;font-size:12px;color:' + this.style.headerFontColor + ';}\n';
    str += '.cdrWatermark{position:absolute;left:0px;top:55px;width:150px;font-family: Arial Black;font-size:50px;color:' + this.style.watermarkColor + ';z-index:1;text-align:center;}\n';
...
Calendar.prototype.getHeader = function(){
    ...
    str += '<td Author="alin" onmouseover="this.className=\'headerOver\'" onmouseout="this.className=\'\'" id="previousYear" title="' + calLang.previousYear + '" style="cursor:pointer;width:10px;" onclick="' + this.Obj + '.onChangeYear(false);"><<</td>\n';
    str += '<td Author="alin" onmouseover="this.className=\'headerOver\'" onmouseout="this.className=\'\'" id="previousMonth" title="' + calLang.previousMonth + '" style="cursor:pointer;width:10px;" onclick="' + this.Obj + '.onChangeMonth(false);"><</td>\n';
    ...
    str += '<td Author="alin" onmouseover="this.className=\'headerOver\'" onmouseout="this.className=\'\'" id="nextMonth" title="' + calLang.nextMonth + '" style="cursor:pointer;width:10px;" onclick="' + this.Obj + '.onChangeMonth(true);">></td>\n';
    str += '<td Author="alin" onmouseover="this.className=\'headerOver\'" onmouseout="this.className=\'\'" id="nextYear" title="' + calLang.nextYear + '" style="cursor:pointer;width:10px;" onclick="' + this.Obj + '.onChangeYear(true);">>></td></tr>\n';
    str += '</table>\n';
    return str;
};
Calendar.prototype.getBody = function(){
    ...
    str += '</table>\n';
    str += '<table Author="alin" class="cdrBodyBar" cellSpacing="2" cellPadding="0"><tr align="center" Author="alin"><td Author="alin" style="cursor:pointer;" onclick="' + this.Obj + '.getToday();">' + calLang.today + '：' + new Date().toFormatString(calLang.dateFormat.replace("MM", "mm")) + '</td></tr></table>\n';
    return str;
};
Calendar.prototype.getBodyBar = function(){
    ...
    var day = simplifyWeek;//new Array('日','一','二','三','四','五','六');
    for (i = 0; i < 7; i++) {
        str += '<td Author="alin_bar">' + day[i].substring(0, 1) + '</td>\n';
    }
    str += '</tr></table>';
    return str;
}
Calendar.prototype.getYearMenu = function(year){
    ...
    for (i = 0; i < 10; i++) {
        ...
        str += 'onclick="' + this.Obj + '.bindDate(\'' + _date.toFormatString("-") + '\')">' + _year + calLang.year + '</td>\n';
        str += '</tr>';
    }
...
Calendar.prototype.getMonthMenu = function(){
    ...
    for (i = 1; i <= 12; i++) {
        ...
        str += 'onclick="' + this.Obj + '.bindDate(\'' + _date.toFormatString("-") + '\')">' + d + '</td></tr>\n';
    }
    str += '</table>';
    var _menu = getObjById("cdrMenu");
    _menu.innerHTML = str;
    ...
    for (i = 0; i < 40; i++) {
        ...
        if (i < _startDay) {
            ...
            if (this.showMoreDay) {
                ...
                _dayElement.title = _previDate.toFormatString(calLang.dateFormat.replace("MM", "mm"));
            ...
            if (_day > _monthDays[_month]) {
                ...
                if (this.showMoreDay) {
                    ...
                    _dayElement.title = _nextDate.toFormatString(calLang.dateFormat.replace("MM", "mm"));
                ...
                if (i >= new Date(_year, _month, 1).getDay() && _day <= _monthDays[_month]) {
                    ...
                    _dayElement.title = _curDate.toFormatString(calLang.dateFormat.replace("MM", "mm"));
...
Calendar.prototype.bindHeader = function(){
    ...
    _curYear.innerHTML = this.date.toFormatString("yyyy" + calLang.year);
    var m = this.date.toFormatString("mm");
    if (m < 10) {
        m = "0" + m;
    }
    _curMonth.innerHTML = m;
    //alert(_curMonth.innerHTML);
...
Calendar.prototype.onClick = function(obj){
	...
	var txt1 = document.getElementById("txt_BeginDate");
	var txt2 = document.getElementById("txt_EndDate");
	if(txt1){
		this.dateInput.value = new Date(this.dateInput.value.replace(/-/g,"/")).format("yyyy-mm-dd");
		var val1= parseInt(txt1.value.replace(/-/g,""));
		var val2= parseInt(txt2.value.replace(/-/g,""));
		var val3 = new Date(txt1.value.replace(/-/g,"/")).format("yyyy-mm-dd");
		var val4 = new Date(txt2.value.replace(/-/g,"/")).format("yyyy-mm-dd");
		if(val1 > val2){
			if(this.dateInput.id == "txt_BeginDate"){
				txt2.value = val3;
				this.dateInput.value = val3;
			}else{
				txt1.value = val4;
				this.dateInput.value = val4;
			}
		}
	}
    this.hide();
